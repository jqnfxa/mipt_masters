...
$D_+ f(x) - f'(x) = \tfrac{h}{2} f''(\xi_+)$. The sign convention in
the statement comes from writing the error as
$D_+ f(x) = f'(x) + O(h)$.
...
$f'''(\xi_0)$ for some $\xi_0$ in that interval, giving the stated
$\tfrac{h^2}{6} f'''(\xi_0)$ form (up to the sign convention).
\end{proof}
