...
Both are standard consequences of Fourier-analytic arguments on~$\R$.
Uniqueness can be shown by smoothing: convolve the law of~$X$ with a
mean-zero Gaussian of variance~$\sigma^2$, so that the smoothed law has
integrable Fourier transform $e^{-\sigma^2 t^2 / 2} \varphi_X(t)$ and the
$L^1$ inversion formula recovers it; letting $\sigma \to 0$, the smoothed
law converges weakly to the law of~$X$, so $\varphi_X$ determines that
law. Inversion under integrability is the standard $L^1$ Fourier
inversion theorem. Lévy's theorem combines pointwise convergence with
Helly's selection theorem and a tightness argument secured by continuity
of~$\varphi$ at~$0$. Full proofs are in
