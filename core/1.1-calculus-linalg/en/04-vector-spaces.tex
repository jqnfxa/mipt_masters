...
(\cref{thm:main-lemma}) all bases of~$V$ contain the same number of
vectors; this common number is the \emph{dimension} of~$V$, denoted
$\dim V$.
\end{definition}
...
linearly independent and $\rank \mat{T} = n$, hence~$\mat{T}$ is
invertible by \cref{thm:cofactor-inverse}. Multiplying
$\vect{x} = \mat{T}\vect{x}'$ by $\mat{T}^{-1}$ gives the second
form.
\end{proof}
...
Define a bilinear pairing on~$\F^n$ by
