...
% --- proof <-> theorem cross-links --------------------------------------------
% Pair: \proofof{key} just after a theorem statement renders a small
% right-aligned hyperlink jumping to the matching proof; \proofanchor{key}
% at the start of that proof drops the hyperref target.
\newcommand{\proofof}[1]{%
  \par\nopagebreak\smallskip\nopagebreak\noindent\hfill%
  \hyperref[prf:#1]{%
    \itshape\small$\hookrightarrow$~\iflanguage{russian}{доказательство}{proof}%
  }%
  \par
}
\newcommand{\proofanchor}[1]{\phantomsection\label{prf:#1}}
...
% Math-house-style macros (\R, \C, …) collide with T1 fontenc accents
% (\C, \L) and a few other packages. babel re-imposes the fontencoding
% on every \selectlanguage, undoing the preamble overrides for those
% names. Reapply via \def here so they survive language switches.
\newcommand{\reapplyMathMacros}{%
  \def\R{\mathbb{R}}\def\C{\mathbb{C}}\def\N{\mathbb{N}}%
  \def\Z{\mathbb{Z}}\def\Q{\mathbb{Q}}\def\F{\mathbb{F}}%
  \def\E{\mathbb{E}}\def\Prob{\mathbb{P}}%
}
